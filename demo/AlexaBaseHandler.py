...
class AlexaBaseHandler(object):
    ...
    def process_request(self, event, context):
        ...
        try:
            ...
            if event['request']['type'] == "LaunchRequest":
                ...
                response = self.on_session_ended(event['request'],
                                                 event['session'])
    ...
    def _build_speechlet_response(self, card_title, card_output, speech_output,
    ...
    def _get_slot_value(self, slot_name, intent_request):
        ...
        try:
            ...
            self.logger.error(
                "Error getting slot value for slot_name={0}".format(slot_name))
